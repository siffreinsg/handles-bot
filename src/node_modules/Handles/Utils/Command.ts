...
export function execute(message: Discord.Message, commandName: string, args: any) {
    ...
    if (commandName !== '') {
        if (app.commands.list.indexOf(commandName) !== -1) {
            ...
            if (argsValid && permsValid) return app.commands.cmds[commandName].execute(new Context(message), givedArgs)
            else {
                if (!permsValid) {
                    let embed = new Discord.RichEmbed()
                        .setColor('#cc0000')
                        .setFooter(app.translate('/misc/requestedBy', message.guild.id, { user: message.author.tag }), message.author.avatarURL)
