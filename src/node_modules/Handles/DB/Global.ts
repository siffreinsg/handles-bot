...
export default class GlobalDB {
    ...
    addServer(serverid: string) {
        this.db.get('servers').push({ id: serverid, users: [], config: { lang: app.config.lang }, stats: { cmdExed: 0, msgSent: 0 } }).write()
    }
    ...
    addUser(serverid: string, userid: string) {
        this.getUsers(serverid).push({ id: userid, xp: 0, balance: 0, stats: { cmdExed: 0, msgSent: 0 }, timestamps: {} }).write()
    }
}
