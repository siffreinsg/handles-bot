...
export default class GlobalDB {
    ...
    createIfNotExists(origin: any, prop, defaultValue: any) {
        let toTest = origin.get(prop)
        if (toTest.value()) return toTest

        origin.set(prop, defaultValue).write()
        return origin.get(prop, defaultValue)
    }
    ...
    /**
         * Get all servers stored in the db
         */
    getServers() {
