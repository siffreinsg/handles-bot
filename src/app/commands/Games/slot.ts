...
export default class Slot extends Command {
    ...
    execute(context: Context, args: Arguments) {
        ...
        if (amount < 10) return context.replyError('custom', context.translate('/commands/slot/amountError'), context.translate('/commands/slot/mustBeAmount'))
        if ((oldBalance - amount) < 0) return context.replyError('custom', context.translate('/commands/slot/notEnoughMoney'), context.translate('/commands/money/cantBeNegative'))
        ...
        switch (true) {
            case (Utils.areEqual(tirage[4], tirage[5], tirage[6])):
                multiplier = 2.5
                toSend += context.translate('/commands/slot/threeNums') + '\n'
                break
            case (tirage[4] === (tirage[5] - 1) && tirage[5] === (tirage[6] - 1)):
                multiplier = 3.5
                toSend += context.translate('/commands/slot/seriesOfNums') + '\n'
                break
        }
        if (lucky === 12) {
            multiplier += 1.5
            toSend += context.translate('/commands/slot/lucky') + '\n'
        }
        ...
        if (multiplier !== 0) {
            ...
            toSend = context.translate('/commands/slot/goodresult', { amount: amount + '', multiplier: multiplier + '', earnings: earnings + '', newBalance: newBalance + '' })
        } else {
            toSend = context.translate('/commands/slot/noEarnings')
        }
